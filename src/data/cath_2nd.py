# from .protein_utils import normalize, safe_index, one_hot_res, log, dihedral, NormalizeProtein, get_stat
# import math
#
...
struc_2nds_res_alphabet3 = ['H', 'E', 'C']
num_struc_2nds_res_alphabet3 = len(struc_2nds_res_alphabet3)
# define a mapping of chars to integers
ss3_vocab = dict((c, i) for i, c in enumerate(struc_2nds_res_alphabet3))
# ss_vocab['PAD'] = len(ss_vocab)

struc_2nds_res_alphabet8 = ['E', 'L', 'I', 'T', 'H', 'B', 'G', 'S']
num_struc_2nds_res_alphabet8 = len(struc_2nds_res_alphabet8)
# define a mapping of chars to integers
ss8_vocab = dict((c, i) for i, c in enumerate(struc_2nds_res_alphabet8))
# ss_vocab['PAD'] = len(ss_vocab)

amino_acids_type = ['A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H', 'I',
                    'L', 'K', 'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V']
aa_vocab = dict((c, i) for i, c in enumerate(amino_acids_type))
# aa_vocab['PAD'] = len(aa_vocab)
rev_aa_vocab = {v: k for k, v in aa_vocab.items()}  # from index to token
...
#     # # define a mapping of chars to integers
#     char_to_int = ss_vocab
#     num_ss = len(struc_2nds_res_alphabet)
#
#     def __init__(self, root: str, name: str = '40', split: str = 'train',
#                  num_residue_type: int = 20,
#                  micro_radius: int = 20,
#                  c_alpha_max_neighbors: int = 10,
#                  cutoff: int = 30,
#                  seq_dist_cut: int = 64,
#                  b_ss_dist_cut: int = 20,
...
#         self.cutoff = cutoff
#         self.b_ss_dist_cut = b_ss_dist_cut
#
...
#
#         self.max_ss_length = max_ss_length
#
#         self.aa_block_token = {} # store the vocab of all aa token block for each ss
#
...
#             torch.save(graph,os.path.join('data/CATH/process', file))
#
#         # save the dictionary
#         aa_block_token = list(self.aa_block_token.keys())
#         aa_dict = dict((c, i) for i, c in enumerate(aa_block_token))
#         rev_aa_dict = {v: k for k, v in aa_dict.items()}
#         torch.save({'aa_dict': aa_dict, 'rev_aa_dict': rev_aa_dict}, 'data/CATH/aa_block_vocab.pt')
#
...
#                         continue
#
#                     # add block features and context features
#                     rec_graph = self.add_block_feat(rec_graph)
#                     rec_graph = self.add_context_feat(rec_graph)
#
#                     print(rec_graph)
...
#             n_coords, c_coords, c_alpha_coords, coord_mask=None, with_coord_mask=False, use_angle=self.use_angle,
...
#             # edge features with multiple scales (controlled by sigma)
...
#         if x is not None:
#             graph = Data(
#                 x=x[:, :-struc_2nd_res.shape[1]],
#                 ss=x[:, -struc_2nd_res.shape[1]:].long(),
...
#                           use_angle=False, use_omega=False):
...
#             src_list) - torch.tensor(dst_list)).reshape(-1, 1) # seq distance using residue index difference
...
#             print(len(data))
#             data = torch.tensor(integer_encoded, dtype=torch.int32).unsqueeze(-1)
...
#
#     ###########################
#     def add_block_feat(self, rec_graph):
#         # if self.max_ss_length is None:
#         #     return False
#
#         ssidx = rec_graph.ss.squeeze()  # [L,]
#         #ssidx = torch.argmax(ss, dim=1, keepdim=False)  # [L,]
#         aa_seq = rec_graph.x[:, :20]
#         aa_seq = torch.argmax(aa_seq, dim=1, keepdim=False) # [L,]
#
#
#         begin = -1
#         end = -1
...
#         segments = []
#         segment_idx = []
#         for i in range(ssidx.shape[0]):
#             # Starting edge case
#             if i == 0:
#                 begin = 0
#                 continue
#
#             if not ssidx[i] == ssidx[i - 1]:
#                 end = i
#                 segments.append(ssidx[i - 1].item())
#                 aa_num.append(end - begin)
#                 segment_idx.append((begin, end))
#                 begin = i
#
#         if not end == ssidx.shape[0]:
#             # last segment is length one
#             segments.append(ssidx[-1].item())
#             aa_num.append(ssidx.shape[0] - begin)
#             segment_idx.append((begin, ssidx.shape[0]))
#
#         assert len(segment_idx) == len(segments) == len(aa_num)
#
#         aa_num = torch.tensor(aa_num, dtype=ssidx.dtype)  # [B,]
#         segments = torch.tensor(segments, dtype=ssidx.dtype)  # [B,]
#         # get ca coords based on segment_idx
#         seg_coord = [0] * segments.shape[0]
#         ca_coords = rec_graph.pos # [L, 3]
#         for ib in range(segments.shape[0]):
#             b_idx = segment_idx[ib]
#             ca_seg_coords = ca_coords[b_idx[0]:b_idx[1], :] # b_aa number of coords
#             seg_coord[ib] = ca_seg_coords.mean(dim=0, keepdim=True)
#
#             # get segment aa block token
#             seg_aa_seq = aa_seq[b_idx[0]:b_idx[1]].tolist()
#             seg_aa_token = ''
#             for seg_aa_idx in seg_aa_seq:
#                 seg_aa_token += rev_aa_vocab[seg_aa_idx]
#             if seg_aa_token in self.aa_block_token.keys():
#                 self.aa_block_token[seg_aa_token] += 1
#             else:
#                 self.aa_block_token[seg_aa_token] = 1
#
#         seg_coord = torch.cat(seg_coord, dim=0)
#
#         #### compute 5-nn graph
#         distances_ = torch.cdist(seg_coord, seg_coord, p=2) # [B, B]
#         distances = distances_ <= self.b_ss_dist_cut
...
#         dst_list = []
#         edges = []
#         for i in range(1, seg_coord.shape[0]):
#             min_dist = {}
#             edges.append((i - 1, i))
#             for j in range(i + 1, seg_coord.shape[0]):
#                 if distances1[i - 1][j] <= 1e-5:  # 不找0, 0都是cutoff被排除的
#                     continue
#                 if len(min_dist.keys()) < 2:
#                     min_dist[j] = distances1[i - 1][j]
#                     continue
#                 if max(min_dist.values()) > distances1[i - 1][j]:
#                     min_dist.pop(list(min_dist.keys())[np.argmax(min_dist.values)])
#                     min_dist[j] = distances1[i - 1][j]
#             for k in list(min_dist.keys()):
#                 edges.append((i - 1, k))
#         for i in range(seg_coord.shape[0] - 2, -1, -1):
#             min_dist = {}
...
#         # cutoff = 6
#         #
#         # # adj matrix based on Calpha TODO: change to Cbeta as in rfdiffusion
#         # dist = torch.cdist(rec_graph.pos, rec_graph.pos, p=2)  # [L,L]
#         # dist[torch.isnan(dist)] = 999.9
#         # dist += 999.9 * torch.eye(ssidx.shape[0])
#         # block_adj = torch.zeros((segments.shape[0], segments.shape[0]))  # [B, B]
#         # for i in range(len(segments)):
#         #     curr_segment = segments[i]
#         #     curr_segment_idx = segment_idx[i]
#         #
#         #     if curr_segment == self.char_to_int['L'] and not include_loops: continue
#         #
#         #     begin_i = curr_segment_idx[0]
#         #     end_i = curr_segment_idx[1]
#         #     for j in range(i + 1, len(segments)):
#         #         j_segment = segments[j]
#         #         j_segment_idx = segment_idx[j]
#         #
#         #         if j_segment == self.char_to_int['L'] and not include_loops: continue
#         #
#         #         begin_j = j_segment_idx[0]
#         #         end_j = j_segment_idx[1]
#         #         if torch.any(dist[begin_i:end_i, begin_j:end_j] < cutoff):
#         #             block_adj[i, j] = torch.ones(1)
#         #             block_adj[j, i] = torch.ones(1)
#
#         rec_graph.b_aa_num = aa_num
#         rec_graph.b_type = segments
#         rec_graph.b_pos = seg_coord
#         rec_graph.b_edge_index, rec_graph.b_edge_attr = edge_indexs, torch.tensor(dist_attr) # dense_to_sparse(block_adj)
#         # rec_graph.b_mask = b_mask
#
#         return rec_graph
#
#
#     def add_context_feat(self, rec_graph):
#
#         b_type = rec_graph.b_type  # [B,]
#         b_aa_num = rec_graph.b_aa_num  # [B,]
#         # b_mask = rec_graph.b_mask # B
#
#         # global information (b_prop, B) 9-dim #TODO: add more
#         B = b_type.shape[0]  # num of blocks
#         b_prop = [0] * self.num_ss
#         for i_s in range(self.num_ss):
#             b_prop[i_s] = ((b_type == i_s) * b_aa_num).sum().item()
#         b_prop = torch.tensor(b_prop, dtype=torch.float32)
#         b_prop = b_prop / b_prop.sum()
#         context = torch.cat((b_prop, torch.tensor([B])))  # [9,]
#
#         rec_graph.context = context
#
#         return rec_graph
...
#############################
# def graph_generate(pdb_filename, edge_index_type):
#     structure = PDBParser().get_structure('random_id', pdb_filename)
#     rec = structure[0]
#     dssp = DSSP(rec, pdb_filename, dssp='mkdssp')
#
#     # 提取所有氨基酸C alpha坐标
#     coords = []
#     for _, chain in enumerate(rec):
#         for _, residue in enumerate(chain):
#             for atom in residue:
#                 if atom.name == 'CA':
#                     c_alpha = list(atom.get_vector())
#                     break
#             coords.append(c_alpha)
#
#     # 对每一个二级结构提取包含的所有氨基酸的C alpha坐标，字典的key为第i个二级结构，value为包含的所有氨基酸坐标均值
#     ss_coords = {}
#     previous_ss = None
#     sec_structures = []
#     ss_coords_array = []
...
#         if j not in ss_coords.keys():
#             ss_coords[j] = []
#         ss_coords[j].append(coords[i])
#     for k, v in ss_coords.items():
#         ss_coords[k] = [sum(e) / len(e) for e in zip(*v)]
#         ss_coords_array.append(ss_coords[k])
#
#     # node特征值
#     ss_coords_array = np.array(ss_coords_array)
...
#     data = F.one_hot(torch.tensor(integer_encoded), num_classes=num_struc_2nds_res_alphabet)
#
#     # 两个ss间距离，边起始点和终止点
#     distances_ = spa.distance.cdist(ss_coords_array, ss_coords_array)
...
#     distances2 = np.tril(distances, k=-1)
#
#     if edge_index_type == "next_knn":
#         src_list = []
#         dst_list = []
#         edges = []
#         # 正向找
#         for i in range(1, len(ss_coords_array)):
#             min_dist = {}
#             edges.append((i - 1, i))
#             for j in range(i + 1, len(ss_coords_array)):
#                 if distances1[i - 1][j] <= 1e-5:  # 不找0, 0都是cutoff被排除的
#                     continue
#                 if len(min_dist.keys()) < 2:
#                     min_dist[j] = distances1[i - 1][j]
...
#                 edges.append((i - 1, k))
#         # 反向找
#         for i in range(len(ss_coords_array) - 2, -1, -1):
#             min_dist = {}
#             if (i, i + 1) not in edges and (i + 1, i) not in edges:
#                 edges.append((i + 1, i))
#             for j in range(i - 1, -1, -1):
...
#             src_list.append(e[0])
#             dst_list.append(e[1])
#         edge_indexs = torch.tensor([src_list, dst_list])
#     elif edge_index_type == "knn":
#         ss_coords_array_tensor = torch.tensor(ss_coords_array)
#         edge_indexs = torch_cluster.knn_graph(ss_coords_array_tensor, k=5)
#     else:
#         raise ValueError("edge_index_type must be knn or next_knn")
#
#     # 边特征值
#     dist_attr = []
#     for i in range(edge_indexs.shape[-1]):
...
#
#     graph = Data(
#         x=data,  # [num_node, node_feature]
#         edge_index=edge_indexs,  # [2, num_edge]
#         edge_attr=torch.tensor(dist_attr),  # [num_edge, edge_feature]
#         distances=torch.tensor(distances),
#         pos=torch.tensor(ss_coords_array)  # [num_node, 3]
#     )
#     return graph
